...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, lengths, targets_feature, feature_mask, bboxes, img_range, ids) in enumerate(data_loader):
    ...
    for i, (images, captions, lengths, targets_feature, feature_mask, bboxes, img_range, ids) in enumerate(data_loader):
        ...
        with torch.no_grad():
            img_emb, bert_embs, cap_len = model.forward_emb(images, captions, lengths, bboxes, img_range)
            cap_feat_emb = model.forward_kvmn(captions, bert_embs, targets_feature, feature_mask)
        if img_embs is None:
            img_embs = np.zeros((len(data_loader.dataset), img_emb.size(1), img_emb.size(2)))
            cap_embs = np.zeros((len(data_loader.dataset), max_n_word, cap_feat_emb.size(2)))
            cap_lens = [0] * len(data_loader.dataset)
            
        # cache embeddings
        img_embs[ids] = img_emb.data.cpu().numpy().copy()
        cap_embs[ids, :max(lengths), :] = cap_feat_emb.data.cpu().numpy().copy()
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    data_loader = get_test_loader(split, opt.data_name, tokenizer, vocab,
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            with torch.no_grad():
                ...
                sim = model.forward_sim(im, ca, l, im_start, ca_start, compare_r_f)
...
if __name__ == '__main__':
    ...
    model_path = "/mnt/data10t/bakuphome20210617/gwx/syntax/test1/results/no49best.tar"
    evalrank(model_path, data_path="/mnt/data10t/bakuphome20210617/lz/data/data1/I-T/Flickr30K", split="test", fold5=False)
